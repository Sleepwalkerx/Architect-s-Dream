package sleepwalker.architectsdream.client.gui.blueprint_creator.screens.types;
...
public class BlockTypeScreen extends BaseCustomScreen {
    ...
    public BlockTypeScreen(ScreenBlueprintCreator parent, ResourceLocation id, DataType type) {
    ...
    public void readData(@Nullable CompoundNBT fileIn) {
        ...
        if(fileIn != null){
            ...
            ListNBT listNBT = fileIn.getList(DATA_LIST, NBTTypes.OBJECT);
            ...
            cacheCheckboxConsider = fileIn.getBoolean(CONSIDER_AIR);
        }
        else {

            cacheCheckboxConsider = Config.CLIENT.considerAir.get();
        }
    }
        ...
        for(BlockElement blockElement : blocks){
            ...
            if(blockElement.mode != PropertyMode.IGNORE){
                ...
                blockElement.typeElements.forEach((blockPos, blockState) -> {
                    ...
                    Map<UBlockPos, Integer> map = entity.validators.get(serializer);

                    if(map == null){
                        map = new HashMap<>();
                        map.put(convertPos(blockPos, fileStructure.minPos), index);
    ...
    @Nonnull
    private UBlockPos convertPos(@Nonnull BlockPos pos, @Nonnull BlockPos minPos){
        return new UBlockPos(pos.getX() - minPos.getX(), pos.getY() - minPos.getY(), pos.getZ() - minPos.getZ());
    }
