package sleepwalker.architectsdream.client.gui.blueprint_maker.utils;
...
public class FileStructureCreator {
    ...
    private static void sendMessage(boolean is, String fileName){
        if(is){
    ...
    public static List<PointTemplateData> getPointTemplateData(CompoundNBT tag){
        if(tag != null && tag.contains(R.BlueprintTemplate.POINTS_DATA, NBTTypes.OBJECT)){
            List<PointTemplateData> data = Lists.newArrayList();
            tag = tag.getCompound(R.BlueprintTemplate.POINTS_DATA);
